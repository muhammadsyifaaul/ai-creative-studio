...
app.post('/api/chat', async (req, res) => {
  ...
  try {
    ...
    if (isGroq) {
      ...
      throw new Error('Invalid API key format. Please use Groq (gsk_) or Anthropic (sk-ant-) API key');
    }
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   ${process.env.HUGGINGFACE_API_KEY ? '✓' : '✗'} Hugging Face API Key ${process.env.HUGGINGFACE_API_KEY ? 'found' : 'missing'}`);
  console.log('');
});